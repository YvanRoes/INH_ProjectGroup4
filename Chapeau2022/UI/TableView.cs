    ...
    public partial class TableView : Form
    {
        ...
        private void btnServeOrder_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else if (lvOrders.SelectedItems[0].SubItems[4].Text == "Running")
                {
                    throw new Exception("This order is not yet ready to serve!");
                }
                else
                {
                    if (lvOrders.SelectedItems[0].SubItems[4].Text == "Finished")
                    {
                        ServeOrderedItem();
        ...
        private void ServeOrderedItem()
        {
            if (lvOrders.SelectedItems.Count > 0)
            {
                foreach (ListViewItem orderedItem in lvOrders.SelectedItems)
                {
                    orderedItemService.MarkOrderedItemAsServed((OrderedItem)orderedItem.Tag);
                }
            }
        }
        ...
        private void CheckTableStatus(int table, out string tableStatus)
        {
            ...
            else if (lvOrders.Items[0].SubItems[4].Text == "Running") //checks if the order is not ready
            {
                tableStatus = "Running";
            }
            else if (lvOrders.Items[0].SubItems[4].Text == "Finished") //checks if the order is ready
            {
